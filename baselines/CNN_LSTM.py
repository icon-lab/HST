# -*- coding: utf-8 -*-
"""
This is the training pipeline for the CNN-LSTM baseline.
...
from keras.layers import Dense, BatchNormalization, Dropout, InputLayer, Bidirectional, MaxPooling1D, Conv1D
...
if __name__ == "__main__":
    ...
    output = open("tasks_cnnlstm.csv", "wb")
    head = [
        "Tasks",
        "Train",
        "Test",
    ...
    for i1 in ["task1"]:
        ...
        for i2 in ["breath_cough"]:  # multi-modal
            ...
            for seed in [1, 2, 5, 12, 40, 52, 72, 2002, 4002, 6002]:
                ...
                optimizer = tf.keras.optimizers.Adam(learning_rate=1e-2) #try clip norm
                ...
                history = model.fit(tensor, y_train, epochs=150, batch_size=32, validation_data=(tensor2,y_test))
