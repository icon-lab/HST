# -*- coding: utf-8 -*-
"""
This is the training pipeline for the BiLSTM baseline.
...
from keras.layers import Dense, BatchNormalization, Dropout, InputLayer, Bidirectional
...
if __name__ == "__main__":
    ...
    output = open("tasks_bilstm.csv", "wb")
    head = [
        "Tasks",
        "Train",
        "Test",
    ...
    for i1 in ["task1"]:
        ...
        for i2 in ["breath_cough"]:  # multi-modal
            ...
            for seed in [1, 2, 5, 12, 40, 52, 72, 2002, 4002, 6002]:
                ...
                train_x = tf.transpose(train_x, perm=[0, 2, 1])
                test_x = tf.transpose(test_x, perm=[0, 2, 1])
                ...
                model.add(Bidirectional(LSTM(50, return_sequences=True, dropout = 0.3)))
                model.add(Bidirectional(LSTM(50, dropout = 0.3)))
                ...
                print("seed: " + str(seed), "acc: ", accuracy, " auc:", aucc, " precision:", precision,
                      " recall", recall, "f1: ", 2*recall*precision/(recall+precision))
